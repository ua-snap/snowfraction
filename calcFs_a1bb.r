...
setwd('/users/stephaniemcafee/Documents/RainSnow/psnow_tave_regions2/FS/a1b2')
...
for (n in 1:5) {
	setwd(paste('/users/stephaniemcafee/documents/RainSnow/psnow_tave_regions2/FS/a1b2/',model[n],sep=''))
	...
	for (s in 1:2) {
  		...
  		fn.se = function(x) {1/(1+exp(-1*(coefs.southcoast[1,s]+coefs.southcoast[2,s]*x)))}
    		...
    		for (i in 1:6) {
      		for (y in 1:9) {
        		tas = raster(paste('/users/stephaniemcafee/Documents/Data/SNAP/771m/decadal/a1b/',model[n],'/tas_decadal_mean_monthly_mean_c_',model[n],'_sresa1b_',m[s,i],'_',decs[y],'.tif',sep=''))
        		...
        		writeRaster(round(fs,0),paste('fs_decadal_mean_monthly_mean_pct_',model[n],'_sresa1b_',m[s,i],'_',decs[y],'.tif',sep=''),format = 'GTiff',options='COMPRESS=LZW',datatype='FLT4S',overwrite=T)
        		rm(tas,fs,fs.ar,fs.in,fs.wc,fs.is,fs.ci,fs.se)
        } #close i
        }#close y
