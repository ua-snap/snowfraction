...
for (i in 1:500) {
#SONDJF
temp1 = as.vector(mtave.10.southcoast[,mos1,i])
temp2 = as.vector(pallsnow.10.southcoast[,mos1,i])
...
#MAMJJA
temp1 = as.vector(mtave.10.southcoast[,mos2,i])
temp2 = as.vector(pallsnow.10.southcoast[,mos2,i])
a = intersect(which(!is.na(temp1)),which(!is.na(temp2)))
clim = data.frame(cbind(temp1[a],temp2[a]))
colnames(clim) = c('tave','psnow')
...
coefs1.southcoast= cbind(b1[,1],b2[,1],p1[,1],p2[,1])
coefs2.southcoast= cbind(b1[,2],b2[,2],p1[,2],p2[,2])
save(list=c('coefs1.southcoast','coefs2.southcoast'),file='coefs.southcoastB.RData')
