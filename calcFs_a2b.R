...
setwd('/users/stephaniemcafee/documents/RainSnow/psnow_tave_regions2')
#setwd('C:\\Users\\smcafee4.UA\\Documents\\RainSnow\\psnow_tave_regions2')
...
setwd('/users/stephaniemcafee/documents/rainsnow/PFE-alaska')
...
#setwd('C:\\Users\\smcafee4.UA\\Documents\\RainSnow\\psnow_tave_regions\\FS\\20c2')
setwd('/users/stephaniemcafee/documents/RainSnow/psnow_tave_regions2/FS/a22')

#set up a matrix of months associated with seasons
m = c('09','10','11','12','01','02')
m=rbind(m,c('03','04','05','06','07','08'))

decs = c('2010_2019','2020_2029','2030_2039','2040_2049','2050_2059','2060_2069','2070_2079','2080_2089','2090_2099')
...
model = c('cccma_cgcm3_1','gfdl_cm2_1','miroc3_2_medres','mpi_echam5','ukmo_hadcm3')

for (n in 1:1) {
	setwd(paste('/users/stephaniemcafee/documents/RainSnow/psnow_tave_regions2/FS/a22/',model[n],sep=''))
	...
	for (s in 2:2) {
  		...
  		fn.se = function(x) {1/(1+exp(-1*(coefs.southcoast[1,s]+coefs.southcoast[2,s]*x)))}
    		...
    		for (i in 6:6) {
      		for (y in 9:9) {
        		tas = raster(paste('/users/stephaniemcafee/Documents/Data/SNAP/771m/decadal/a2/',model[n],'/tas_decadal_mean_monthly_mean_c_',model[n],'_sresa2_',m[s,i],'_',decs[y],'.tif',sep=''))
        		...
        		writeRaster(round(fs,0),paste('fs_decadal_mean_monthly_mean_pct_',model[n],'_sresa2_',m[s,i],'_',decs[y],'.tif',sep=''),format = 'GTiff',options='COMPRESS=LZW',datatype='FLT4S',overwrite=T)
        		rm(tas,fs,fs.ar,fs.in,fs.wc,fs.is,fs.sw,fs.ci,fs.se)
        } #close i
        }#close y
